...
type WebSocketMessage struct {
	...
	Code    string                 `json:"code"` // For backward compatibility
	Channel string                 `json:"channel,omitempty"`
...
func closeConnection(client *Client) {
	// Set close code to normal closure
...
func RegisterRoutes(app fiber.Router, service domainApp.IAppUsecase) {
	...
	app.Get("/ws", websocket.New(func(conn *websocket.Conn) {
		...
		conn.SetCloseHandler(func(code int, text string) error {
			logrus.Infof("[WS] Client %s initiated close: %d - %s", client.ID, code, text)
			return nil
		})
		...
		for {
			...
			if err != nil {
				if websocket.IsUnexpectedCloseError(err, websocket.CloseGoingAway, websocket.CloseAbnormalClosure, websocket.CloseNormalClosure) {
					...
					logrus.Infof("[WS] Client %s disconnected: %v", client.ID, err)
				}
				return
			}
			...
			if messageType == websocket.TextMessage {
				...
				if err := conn.WriteMessage(websocket.PongMessage, message); err != nil {
					logrus.Errorf("[WS] Failed to send pong to client %s: %v", client.ID, err)
					return
				}
			} else {
				logrus.Debugf("[WS] Received message type %d from client %s", messageType, client.ID)
			}
		}
	}))
}
...
func handleClientMessage(client *Client, message []byte, service domainApp.IAppUsecase) {
	...
	// Use Code if Type is not available (backward compatibility)
	messageType := wsMessage.Type
	if messageType == "" {
		messageType = wsMessage.Code
	}
	...
	default:
		logrus.Debugf("[WS] Unknown message type from client %s: %s", client.ID, messageType)
	}
}
...
func startPingTicker() {
	...
	for range ticker.C {
		clientsMux.RLock()
		for _, client := range Clients {
			// Check if client is still alive (last ping within 90 seconds)
			if time.Since(client.LastPing) > 90*time.Second {
				logrus.Warnf("[WS] Client %s appears to be dead (no ping for 90s), closing connection", client.ID)
				closeConnection(client)
				continue
			}
