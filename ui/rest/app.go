...
func InitRestApp(app fiber.Router, service domainApp.IAppUsecase) App {
	...
	app.Get("/app/session/health", rest.SessionHealth)
...
func (handler *App) Login(c *fiber.Ctx) error {
	...
	return c.JSON(utils.ResponseData{
		...
		Results: map[string]any{
			...
			"qr_base64":   response.ImageBase64,
...
func (handler *App) SessionHealth(c *fiber.Ctx) error {
	health, err := handler.Service.GetSessionHealth(c.UserContext())
	utils.PanicIfNeeded(err)

	return c.JSON(utils.ResponseData{
		Status:  200,
		Code:    "SUCCESS",
		Message: "Session health retrieved",
		Results: health,
	})
}
