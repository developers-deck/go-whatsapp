...
// CleanupDatabase handles database cleanup for both SQLite and PostgreSQL
func CleanupDatabase() error {
	// Check if we're using PostgreSQL
	if strings.HasPrefix(config.DBURI, "postgres://") || strings.HasPrefix(config.DBURI, "postgresql://") {
		logrus.Info("[CLEANUP] Using PostgreSQL - skipping file cleanup")
		// For PostgreSQL, we don't remove files, just truncate tables
		return nil
	}

	// Handle SQLite file cleanup
