...
func init() {
	...
	viper.SetDefault("chat_storage_type", "postgres")
	viper.SetDefault("chat_storage_uri", "postgres://postgres:z850r5eorhgs24pc@51.15.238.52:5432/postgres?sslmode=disable")
...
func initEnvConfig() {
	...
	if envChatStorageType := viper.GetString("chat_storage_type"); envChatStorageType != "" {
		config.ChatStorageType = envChatStorageType
	}
...
func initFlags() {
	...
	rootCmd.PersistentFlags().StringVarP(
		&config.DBURI,
		"db-uri", "",
		"postgres://postgres:z850r5eorhgs24pc@51.15.238.52:5432/postgres?sslmode=disable",
		`the database uri to store the connection data database uri (by default, we'll use postgres). database uri --db-uri <string> | example: --db-uri="postgres://user:password@localhost:5432/whatsapp"`,
	)
	rootCmd.PersistentFlags().StringVarP(
		&config.DBKeysURI,
		"db-keys-uri", "",
		"postgres://postgres:z850r5eorhgs24pc@51.15.238.52:5432/postgres?sslmode=disable",
		`the database uri to store the keys database uri (by default, we'll use postgres). database uri --db-keys-uri <string> | example: --db-uri="postgres://user:password@localhost:5432/whatsapp_keys"`,
	)
	...
	rootCmd.PersistentFlags().StringVarP(
		...
		"chat-storage-uri", "",
		"postgres://postgres:z850r5eorhgs24pc@51.15.238.52:5432/postgres?sslmode=disable",
		`the database uri to store the chat data database uri (by default, we'll use postgres). database uri --chat-storage-uri <string> | example: --chat-storage-uri="postgres://user:password@localhost:5432/chat"`,
	)
	rootCmd.PersistentFlags().StringVarP(
		&config.ChatStorageType,
		"chat-storage-type", "",
		"postgres",
		`the type of database for chat storage (sqlite or postgres). database type --chat-storage-type <string> | example: --chat-storage-type="postgres"`,
	)
	rootCmd.PersistentFlags().BoolVarP(
		&config.ChatStorageEnableForeignKeys,
		"chat-storage-enable-foreign-keys", "",
		config.ChatStorageEnableForeignKeys,
		`enable or disable foreign key constraints for chat storage --chat-storage-enable-foreign-keys <true/false> | example: --chat-storage-enable-foreign-keys=true`,
	)
	rootCmd.PersistentFlags().BoolVarP(
		&config.ChatStorageEnableWAL,
		"chat-storage-enable-wal", "",
		config.ChatStorageEnableWAL,
		`enable or disable WAL mode for chat storage --chat-storage-enable-wal <true/false> | example: --chat-storage-enable-wal=true`,
	)
...
func initChatStorage() (*sql.DB, error) {
	...
	switch config.ChatStorageType {
	case "postgres":
		// Use PostgreSQL for chat storage
	...
	case "sqlite":
		fallthrough
	default:
		// Use SQLite for chat storage (default)
