export default {
    ...
    methods: {
        async loadWebhooks() {
            this.loading = true;
            try {
                const response = await window.http.get('/webhooks');
                this.webhooks = response.data.results?.endpoints || [];
                
                const deliveriesResponse = await window.http.get('/webhooks/stats');
