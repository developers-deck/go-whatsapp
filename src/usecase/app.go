...
import (
	...
	"github.com/aldinokemal/go-whatsapp-web-multidevice/pkg/session"
...
func NewAppService(chatStorageRepo domainChatStorage.IChatStorageRepository) domainApp.IAppUsecase {
	...
	service.sessionManager.StartPeriodicBackup(ctx, func() session.SessionInfo {
		...
		return session.SessionInfo{
			ID:          "main", // For single instance, use "main" as session ID
			DeviceID:    deviceID,
			PushName:    pushName,
		...
		go func() {
			for evt := range ch {
				...
				if evt.Event == "code" {
					...
					if err != nil {
						logrus.Error("Error when generate qr code bytes: ", err)
						chImage <- ""
						return
					}
					...
					response.ImageBase64 = "data:image/png;base64," + qrBase64
...
func (service *serviceApp) GetSessionHealth(ctx context.Context) (response map[string]interface{}, err error) {
	health := service.sessionManager.GetSessionHealth(ctx)
	
	// Add current client status
	client := whatsapp.GetClient()
	if client != nil {
		health["client_connected"] = client.IsConnected()
		health["client_logged_in"] = client.IsLoggedIn()
		
		if client.Store.ID != nil {
			health["device_id"] = client.Store.ID.String()
		}
		if client.Store.PushName != "" {
