export default {
    ...
    methods: {
        async loadProcesses() {
            this.loading = true;
            try {
                const response = await window.http.get('/monitor/processes');
                this.processes = response.data.results || [];
                
                const statsResponse = await window.http.get('/monitor/system');
        ...
        async restartProcess(pid) {
            try {
                await window.http.post(\`/monitor/processes/\${pid}/restart\`);
        ...
        async killProcess(pid) {
            if (confirm('Are you sure you want to kill this process?')) {
                try {
                    await window.http.post(\`/monitor/processes/\${pid}/kill\`);
